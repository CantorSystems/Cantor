...
const
  ...
  sModuleAccessViolation = 'Access violation at address %p in module У%sФ:%hs%hs of address %p';
  ...
  sVersionAndBuild = '%s сборка %u';
  sVersionAndRevision = '%s ревизи€ %u';
  ...
  sInvalidInteger = 'У%sФ is not a valid %hs value';
  ...
  sSystemCPtoUTF16 = 'System error У%sФ while converting code page %u (%s) to UTF-16';
  sSystemUTF16toCP = 'System error У%sФ while converting UTF-16 to code page %u (%s)';
  ...
  sInvalidImageStub = 'Invalid image stub Ц %u bytes on last page (max 512 allowed)';
  ...
  sUnsupportedExeImage = 'Unsupported image extension С%c%cТ';
  ...
  sAvoidCharCorruption = 'Ёта программа использует консоль UTF-8. „тобы предотвратить искажение' + sLineBreak +
    'символов Unicode, установите дл€ окна консоли шрифт TrueType/OpenType.';
  sPressEnterToExit = 'Ќажмите ENTER дл€ выхода';
  ...
  sComment = 'comment';
  sNotUnderstandable = '%hs У%sФ is not understandable for international community';
  sNotLocalizable = 'string У%sФ is not localizable';
  sDependsOnLocale = 'source file У%sФ depends on locale character set';
