...
//  LockTest;
{  CharSetTest;
  SingleStringTest('Mother Û¯Î‡ Ì‡ ‡·ÓÚÛ, ‡ ÔÓÚÓÏ ‚ÂÌÛÎ‡Ò¸ ‰ÓÏÓÈ.');
  SingleStringTest(Malformed, 2);
  //SingleStringTest(Surrogate);
  RandomStringTest($FFFF, $FFFF div 64, $400);}
  FConsole.CodePage := 1251;
  P := Format('My string "%s" at %i', [PLegacyChar('—“–Œ ¿'), 9]);
  FConsole.WriteLn(P);
  FreeMemAndNil(P);
//  raise ESharingViolation.Create(Self, opSyncUpdate);
//  raise EIndex.Create(Self, -1, 0, 9);
//  W := LatinFormat('My string "%S" at %i', [PLegacyChar('—“–Œ ¿'), 9]);
//  FreeMemAndNil(W);
  W := LegacyFormat('My string "%S" at %i', CP_ACP, [PLegacyChar('—“–Œ ¿'), 9]);
  FreeMemAndNil(W);
  W := WideFormat('My string "%S" at %i', [PLegacyChar('—“–Œ ¿'), 9]);
  FreeMemAndNil(W);
//  A := PCardinal(W)^;
//  P^ := LegacyChar(A);
end;
